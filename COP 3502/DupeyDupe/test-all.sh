...
if ! [ -x "$(command -v grep)" ]; then
	...
	num_spc_lines=`grep "^ " DupeyDupe.c | wc -l`
	num_tab_lines=`grep "$(printf '^\t')" DupeyDupe.c | wc -l`
	num_und_lines=`grep "$(printf '^[^\t ]')" DupeyDupe.c | wc -l`
	...
	if [ "$num_spc_lines" -gt 0 ] && [ "$num_tab_lines" -gt 0 ]; then
		...
		echo ""
		echo "     grep \"^ \" DupeyDupe.c -n"
		echo "     grep \"\$(printf '^[\t ]')\" DupeyDupe.c -n"
